...
namespace CheckMxAviationWeather.Api.Factories
{
    internal class StationFactory
    {
        ...
        /// <returns></returns>
        internal List<Station> BuildMultiple(string json)
        {
            var returnValue = new List<Station>();
            ...
            foreach (var o in asObj.Data)
            {
                ...
                try
                {
                    returnValue.Add(JsonConvert.DeserializeObject<Station>(Convert.ToString(o)));
                }
                catch (Exception)
                {
                    // IGNORED!
